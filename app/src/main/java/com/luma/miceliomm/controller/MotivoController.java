...
private MotivoMaestroAdapter adapter;
...
public MotivoController(Context context) {
    ...
    this.dao = new MotivoDao(db);
}

public MotivoController(Context context, RecyclerView recyclerView, MotivoMaestroAdapter adapter, SwipeRefreshLayout swipeRefreshLayout, LinearLayout recyclerViewEmpty) {
    this.context = context;
...
public void getDatos(TextView Total) {
    ...
    Call<ArrayList<MotivoModel>> call = apiService.getMotivos();
    call.enqueue(new Callback<ArrayList<MotivoModel>>() {
        @Override
        public void onResponse(Call<ArrayList<MotivoModel>> call, Response<ArrayList<MotivoModel>> response) {
            if (response.isSuccessful() && !response.body().isEmpty()) {
                dao.delete();
                pDialog.setTitle("Guardando Datos");
                pDialog.setMessage("Espere...");
                for (MotivoModel cls : response.body()) {
                    dao.insert(cls);
                }
                util.msgSnackBar("Descarga correctamente", context);
                Total.setText("Total: " + dao.totalRegistros());
            } else {
                util.mensaje("Error al descargr Motivos (Response Code Referencia:  " + response.code() + ")", ((Activity) context)).show();
            }
            if (pDialog.isShowing()) pDialog.dismiss();
        }

        @Override
        public void onFailure(Call<ArrayList<MotivoModel>> call, Throwable t) {
