...
public class PilotoMaestroAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {

    public List<PilotoModel> info;
    public List<PilotoModel> aux;
    ...
    public PilotoMaestroAdapter(List<PilotoModel> info, Context context,  OnItemClickListener mItemClickListener) {
        this.info = info;
    ...
    public void setInfo(List<PilotoModel> info) {
        this.info = info;
        this.aux = new ArrayList<>(info);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemAdapterViewHolder) {
            final PilotoModel cls = info.get(position);
            ItemAdapterViewHolder holderItem = (ItemAdapterViewHolder) holder;
            holderItem.lblCVIdMaestro.setText(String.valueOf(cls.IdPersonal));
    ...
    private Filter getBuscar = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            List<PilotoModel> filteredList = new ArrayList<>();
            if (charSequence == null || charSequence.length() == 0) {
                filteredList.addAll(aux);
            } else {
                String filterPattern = charSequence.toString().toLowerCase().trim();
                for (PilotoModel item : aux) {
