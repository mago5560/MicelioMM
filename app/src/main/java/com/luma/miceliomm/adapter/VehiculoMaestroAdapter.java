...
public class VehiculoMaestroAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {

    public List<VehiculoModel> info;
    public List<VehiculoModel> aux;
    ...
    public VehiculoMaestroAdapter(List<VehiculoModel> info, Context context, OnItemClickListener mItemClickListener) {
        this.info = info;
    ...
    public void setInfo(List<VehiculoModel> info) {
        this.info = info;
        this.aux = new ArrayList<>(info);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemAdapterViewHolder) {
            final VehiculoModel cls = info.get(position);
            ItemAdapterViewHolder holderItem = (ItemAdapterViewHolder) holder;
            holderItem.lblCVIdMaestro.setText(String.valueOf(cls.IdVehiculo));
            holderItem.lblCVTitulo.setText("Placa");
            holderItem.lblCVTituloDescripcion.setText(cls.Placa);
            holderItem.lblCVSubTitulo.setText("Descripcion");
            holderItem.lblCVSubTituloDescripcion.setText(cls.Nombre);
    ...
    private Filter getBuscar = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            List<VehiculoModel> filteredList = new ArrayList<>();
            if (charSequence == null || charSequence.length() == 0) {
                filteredList.addAll(aux);
            } else {
                String filterPattern = charSequence.toString().toLowerCase().trim();
                for (VehiculoModel item : aux) {
