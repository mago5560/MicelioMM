...
public class TipoDocumentoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {

    public List<TipoDocumentoModel> info;
    public List<TipoDocumentoModel> aux;
    ...
    public TipoDocumentoAdapter(List<TipoDocumentoModel> info, Context context,  OnItemClickListener mItemClickListener) {
        this.info = info;
    ...
    public void setInfo(List<TipoDocumentoModel> info) {
        this.info = info;
        this.aux = new ArrayList<>(info);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemAdapterViewHolder) {
            final TipoDocumentoModel cls = info.get(position);
            ItemAdapterViewHolder holderItem = (ItemAdapterViewHolder) holder;
            holderItem.lblCVIdMaestro.setText(String.valueOf(cls.id));
    ...
    private Filter getBuscar = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            List<TipoDocumentoModel> filteredList = new ArrayList<>();
            if (charSequence == null || charSequence.length() == 0) {
                filteredList.addAll(aux);
            } else {
                String filterPattern = charSequence.toString().toLowerCase().trim();
                for (TipoDocumentoModel item : aux) {
